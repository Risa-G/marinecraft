...
import net.neoforged.neoforge.registries.{DeferredBlock, DeferredHolder, DeferredItem, DeferredRegister, NeoForgeRegistries}
...
object ExampleMod {
  ...
  val EXAMPLE_BLOCK: DeferredBlock[Block] = BLOCKS.register("example_block", () => new Block(BlockBehaviour.Properties.of().mapColor(MapColor.STONE)))
  val EXAMPLE_BLOCK_ITEM: DeferredItem[BlockItem] = ITEMS.register("example_block", () => new BlockItem(EXAMPLE_BLOCK.get(), new Item.Properties()))

  val EXAMPLE_ITEM: DeferredItem[Item] = ITEMS.register("example_item", () => new Item(new Item.Properties().food(new FoodProperties.Builder()
    .alwaysEdible().nutrition(1).saturationModifier(2f).build())))

  val EXAMPLE_TAB: DeferredHolder[CreativeModeTab, CreativeModeTab] = CREATIVE_MODE_TABS.register("example_tab", () => CreativeModeTab.builder()
  ...
  implicit val showId: Show[ModID] = (t: ModID) => {
    val name = ModList.get().getModContainerById(t.id).map[String](o => o.getModInfo.toString).orElse("None")
  ...
  @EventBusSubscriber(modid = MOD_ID, bus = EventBusSubscriber.Bus.MOD, value = Array(Dist.CLIENT))
